...
function ReviewCard({ review }: { review: Review }) {
  const displayName = review.userId === "user-1" ? "You" : review.userId;
...
export default function ShopDetailScreen({ shop, reviews, onBack, onBookNow }: Props) {
  ...
  const seededReviews = INITIAL_REVIEWS.filter((review) => review.shopId === shop.id);
  const recentReviews = [...submittedReviews, ...seededReviews].slice(0, 4);
  const totalReviewCount = shop.reviewCount + submittedReviews.length;
  const totalRatingScore =
    shop.rating * shop.reviewCount +
    submittedReviews.reduce((total, review) => total + review.rating, 0);
  const averageRating = totalReviewCount > 0 ? totalRatingScore / totalReviewCount : shop.rating;
      ...
      <div className="flex flex-col gap-3 px-4 pt-4">
        ...
        <div className="rounded-[16px] bg-card p-4 card-shadow">
          ...
          {recentReviews.length === 0 ? (
            ...
            recentReviews.map((review) => <ReviewCard key={review.reviewId} review={review} />)
          )}
        </div>
      </div>
      ...
      >
        <div className="mb-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm font-semibold text-foreground">{shop.nextSlot} Today</p>
          </div>
